...
function sanitizeInput(input) {
    ...
    let sanitized = input
        ...
        .replace(/on\w+='[^']*'/gi, '') // Remove inline event handlers (e.g., onclick='')
        .replace(/<.*?on\w+=.*?>/gi, match => match.replace(/on\w+=["'][^"']*["']/gi, '')) // Remove event handlers in tags
        .trim();
    return sanitized;
}

// For production, consider using a robust library like DOMPurify for comprehensive sanitization:
